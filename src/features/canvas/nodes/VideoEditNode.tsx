...
const VIDEO_EDIT_NODE_MIN_WIDTH = 390;
const VIDEO_EDIT_NODE_MIN_HEIGHT = 180;
const VIDEO_EDIT_NODE_MAX_WIDTH = 1400;
const VIDEO_EDIT_NODE_MAX_HEIGHT = 1000;
const VIDEO_EDIT_NODE_DEFAULT_WIDTH = 520;
const VIDEO_EDIT_NODE_DEFAULT_HEIGHT = 320;

const DEFAULT_VIDEO_MODEL_ID = '12ai-veo/veo_3_1-fast';
...
function pickClosestAspectRatio(
  ...
  const supported = supportedAspectRatios.length > 0 ? supportedAspectRatios : ['16:9'];
...
export const VideoEditNode = memo(({ id, data, selected, width, height }: VideoEditNodeProps) => {
  ...
  const providerConfigs = useSettingsStore((state) => state.providerConfigs);
  ...
  const allModels = useMemo(() => listImageModels(), []);
  const videoModels = useMemo(
    () => allModels.filter((m) => m.mediaType === 'video'),
    [allModels]
  );
  ...
  const selectedAspectRatio = useMemo(
    () =>
      ...
      aspectRatioOptions.find((item) => item.value === '16:9') ??
  ...
  const requestResolution = useMemo(
    ...
    [incomingImages.length, selectedModel]
  );
  const resolvedRequestModel = useMemo(
    ...
    [providerConfigs, requestResolution.requestModel, selectedModel.providerId]
  );
  ...
  const resolvedTitle = useMemo(
    () => resolveNodeDisplayName(CANVAS_NODE_TYPES.videoEdit, data),
    [data]
  );

  const resolvedWidth = Math.max(VIDEO_EDIT_NODE_MIN_WIDTH, Math.round(width ?? VIDEO_EDIT_NODE_DEFAULT_WIDTH));
  const resolvedHeight = Math.max(VIDEO_EDIT_NODE_MIN_HEIGHT, Math.round(height ?? VIDEO_EDIT_NODE_DEFAULT_HEIGHT));
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (!prompt && incomingImages.length === 0) {
      const errorMessage = t('node.videoEdit.promptRequired');
    ...
    if (!providerApiKey) {
      const errorMessage = t('node.videoEdit.apiKeyRequired');
      setError(errorMessage);
      void showErrorDialog(errorMessage, t('common.error'));
      return;
    }
    ...
    const resultNodeTitle = buildAiResultNodeTitle(prompt || '视频生成', t('node.videoEdit.resultTitle'));
    ...
    try {
      await canvasAiGateway.setProviderConfig({
        ...
        preferAsync: providerConfigs[selectedModel.providerId]?.preferAsync ?? true,
      });
      ...
      if (resolvedRequestAspectRatio === AUTO_REQUEST_ASPECT_RATIO) {
        if (incomingImages.length > 0) {
          try {
            ...
            resolvedRequestAspectRatio = pickClosestAspectRatio(1.777, supportedAspectRatioValues);
          }
        } else {
          resolvedRequestAspectRatio = pickClosestAspectRatio(1.777, supportedAspectRatioValues);
        }
    ...
    >
      ...
      <div className="mt-2 flex shrink-0 items-center gap-1">
        ...
        <UiButton
          ...
          variant="primary"
          className={`shrink-0 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white shadow-lg shadow-purple-500/20 ${NODE_CONTROL_PRIMARY_BUTTON_CLASS}`}
        >
      ...
      <NodeResizeHandle
        ...
        maxWidth={VIDEO_EDIT_NODE_MAX_WIDTH}
        maxHeight={VIDEO_EDIT_NODE_MAX_HEIGHT}
      />
    </div>
  );
});
