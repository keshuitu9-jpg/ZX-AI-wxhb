...
import { AlertTriangle, Globe, Image as ImageIcon, Maximize2, Sparkles } from 'lucide-react';
...
import { PanoramaInlineViewer } from '@/features/canvas/ui/PanoramaInlineViewer';
...
export const ImageNode = memo(({ id, data, selected, type, width, height }: ImageNodeProps) => {
  ...
  const isVideoResult = (data as any).generationSourceType === 'videoEdit' ||
    (typeof data.imageUrl === 'string' && (data.imageUrl.endsWith('.mp4') || data.imageUrl.endsWith('.webm') || data.imageUrl.endsWith('.mov')));
  ...
  const waitingResultText = useMemo(() => {
    ...
    const baseText = isVideoResult ? '等待生成视频...' : t('node.imageNode.waitingResult');
  ...
  }, [isExportResultNode, isGenerating, isVideoResult, t, waitedMinutes]);
      ...
      >
        {data.imageUrl ? (
          isVideoResult ? (
            ...
            <>
              {(data as any)._panoramaMode ? (
                <PanoramaInlineViewer
                  imageUrl={originalImageUrl ?? imageSource ?? ''}
                  width={resolvedWidth}
              ...
              {/* 360° 全景切换按钮 */}
              <button
                className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1 px-2 py-1 rounded-md bg-black/60 text-white text-[11px] backdrop-blur-sm hover:bg-black/80 border border-white/20"
                title={(data as any)._panoramaMode ? '退出全景' : '360° 全景查看'}
                onClick={(e) => {
                  e.stopPropagation();
                  updateNodeData(id, { _panoramaMode: !(data as any)._panoramaMode });
                }}
              >
                <Globe className="h-3.5 w-3.5" />
                <span>{(data as any)._panoramaMode ? '退出' : '360°'}</span>
              </button>
              {/* 全景模式下的全屏按钮 */}
              {(data as any)._panoramaMode && (
                <button
                  className="absolute bottom-2 left-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center gap-1 px-2 py-1 rounded-md bg-black/60 text-white text-[11px] backdrop-blur-sm hover:bg-black/80 border border-white/20"
                  title="全屏查看"
                  onClick={(e) => {
                    e.stopPropagation();
                    const openPanorama = useCanvasStore.getState().openPanoramaViewer;
                    if (originalImageUrl) {
                      openPanorama(originalImageUrl);
                    }
                  }}
                >
          ...
          <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-text-muted/85">
            {isExportResultNode ? (
              isVideoResult ? (
                <Sparkles className="h-7 w-7 opacity-60 text-purple-400" />
              ) : (
