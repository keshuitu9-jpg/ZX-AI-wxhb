...
export const StoryboardNode = memo(({ id, data, selected, width, height }: StoryboardNodeProps) => {
  ...
  const availableImageItems = useMemo<IncomingImageItem[]>(() => {
    const connectedSourceNodeIds = new Set(
      edges.filter((edge) => edge.target === id).map((edge) => edge.source)
    );
    const dedupedByImageUrl = new Map<string, IncomingImageItem & { priority: number }>();

    const registerCandidate = (
      candidate: Omit<IncomingImageItem, 'displayUrl'> & { priority: number }
    ) => {
      const existing = dedupedByImageUrl.get(candidate.imageUrl);
      if (existing && existing.priority <= candidate.priority) {
        return;
      }

      dedupedByImageUrl.set(candidate.imageUrl, {
        ...candidate,
        displayUrl: resolveImageDisplayUrl(candidate.previewImageUrl || candidate.imageUrl),
      });
    };
    ...
    for (const node of nodes) {
      if (isUploadNode(node) || isImageEditNode(node) || isExportImageNode(node)) {
        ...
        registerCandidate({
          ...
          priority: connectedSourceNodeIds.has(node.id) ? 0 : 1,
        });
        continue;
      }
      ...
      const splitTitle = resolveNodeDisplayName(node.type, node.data);
      node.data.frames.forEach((frame, index) => {
        const imageUrl = frame.imageUrl ?? frame.previewImageUrl ?? null;
        if (!imageUrl) {
          return;
        }

        registerCandidate({
          id: `frame:${node.id}:${frame.id}`,
          imageUrl,
          previewImageUrl: frame.previewImageUrl ?? null,
          label: `${splitTitle} · 鍒嗛暅 ${String(index + 1).padStart(2, '0')}`,
          sourceNodeId: node.id,
          sourceFrameId: frame.id,
          priority: node.id === id ? 2 : 3,
        });
      });
    }
  ...
  const incomingImageRefs = availableImageItems;
      ...
      <div className="mt-2 flex shrink-0 items-center justify-between gap-2">
        <div className="flex min-w-0 flex-1 items-center gap-2 overflow-hidden">
          <div ref={exportSettingsTriggerRef} className="nodrag relative flex">
            <UiChipButton
              ...
              className={`${NODE_CONTROL_CHIP_CLASS} !shrink-0 whitespace-nowrap`}
              onClick={(event) => {
                event.stopPropagation();
                if (isExportPanelOpen) {
                  setIsExportPanelOpen(false);
          ...
          <div className="min-w-0 truncate whitespace-nowrap text-[11px] text-text-muted/80">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <UiButton
            size="sm"
            variant="muted"
            className={`nodrag ${NODE_CONTROL_PRIMARY_BUTTON_CLASS} !min-w-[88px] !shrink-0 whitespace-nowrap`}
            onClick={(event) => {
              event.stopPropagation();
