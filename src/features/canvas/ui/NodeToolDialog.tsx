...
import { ChromaKeyToolEditor } from './tool-editors/ChromaKeyToolEditor';
...
export function NodeToolDialog() {
  ...
  const widthClassName = useMemo(() => {
    ...
    if (activePlugin.editor === 'chromaKey') {
      return 'w-[min(1020px,calc(100vw-40px))]';
    }
    return 'w-[min(460px,calc(100vw-40px))]';
  }, [activePlugin]);
  ...
  const editorContent = useMemo(() => {
    ...
    if (activePlugin.editor === 'chromaKey' && sourceImageUrl) {
      return (
        <ChromaKeyToolEditor
          plugin={activePlugin}
          sourceImageUrl={sourceImageUrl}
