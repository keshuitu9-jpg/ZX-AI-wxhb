...
export function CropToolEditor({ plugin, sourceImageUrl, options, onOptionsChange }: VisualToolEditorProps) {
  ...
  const renderedImageSize = useMemo(() => {
    ...
    return {
      width: Math.max(1, Math.round(naturalSize.width * ratio * zoom)),
      height: Math.max(1, Math.round(naturalSize.height * ratio * zoom)),
    };
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-text-muted/70">Ctrl+滚轮缩放图片</span>
      ...
      <div
        ref={viewportRef}
        className="relative h-[min(62vh,640px)] overflow-auto rounded-xl border border-[rgba(255,255,255,0.12)] bg-bg-dark/85"
        onWheel={handleWheel}
      >
