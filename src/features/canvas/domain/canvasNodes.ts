...
export interface PromptNodeData extends NodeDisplayData {
  prompts: PromptItem[];
  [key: string]: unknown;
}
...
export interface VideoEditNodeData extends ImageEditNodeData {}
...
export function isPromptNode(
  node: CanvasNode | null | undefined
): node is Node<PromptNodeData, typeof CANVAS_NODE_TYPES.prompt> {
  return node?.type === CANVAS_NODE_TYPES.prompt;
}
...
export function nodeHasImage(node: CanvasNode | null | undefined): boolean {
  ...
  if (isUploadNode(node) || isImageEditNode(node) || isVideoEditNode(node) || isExportImageNode(node)) {
