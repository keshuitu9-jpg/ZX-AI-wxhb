...
export async function loadImageElement(source: string): Promise<HTMLImageElement> {
  ...
  return await new Promise((resolve, reject) => {
    ...
    image.onerror = () => {
      if (displaySource.startsWith('asset:') || displaySource.startsWith('file:') || isLikelyLocalImagePath(source)) {
        console.warn(`[imageData] Failed to load local image element: ${source}, using fallback.`);
        const fallbackImage = new Image();
        fallbackImage.onload = () => resolve(fallbackImage);
        fallbackImage.onerror = () => reject(
          createImagePipelineError('图片加载失败 (Fallback error)', `source=${source}\ndisplaySource=${displaySource}`)
        );
        fallbackImage.src = TRANSPARENT_1X1_PNG_DATA_URL;
        return;
      }
      reject(
...
export const TRANSPARENT_1X1_PNG_DATA_URL = 'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII=';
  ...
  if (isLikelyLocalImagePath(imageUrl)) {
    if (isTauri()) {
      try {
        ...
        if (msg.includes('os error 2') || msg.includes('os error 3') || msg.includes('系统找不到指定的')) {
          console.warn(`[imageData] Local file missing: ${imageUrl}, using placeholder`, error);
          return TRANSPARENT_1X1_PNG_DATA_URL;
        }
