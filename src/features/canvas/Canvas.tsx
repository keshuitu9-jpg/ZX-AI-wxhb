...
import { AIChatbot } from '@/features/chat/ui/AIChatbot';
...
export function Canvas() {
  ...
  const panoramaViewer = useCanvasStore((state) => state.panoramaViewer);
  const closePanoramaViewer = useCanvasStore((state) => state.closePanoramaViewer);
            ...
            if (generationProviderId) {
              ...
              await canvasAiGateway.setProviderConfig({
                ...
                model: resolveProviderRequestModel(
                  ...
                  providerConfigs
                ).replace(`${generationProviderId}/`, ''),
                preferAsync: providerConfig?.preferAsync ?? true,
              }).catch((error) => {
                console.warn('[GenerationJob] set_provider_config failed before poll', {
  ...
  }, [apiKeys, nodes, providerConfigs, updateNodeData]);
