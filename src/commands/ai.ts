...
function assertTauriReady(): void {
  if (!isTauri()) {
    throw new Error('褰撳墠涓嶆槸 Tauri 瀹瑰櫒鐜锛岃浣跨敤 `npm run tauri dev` 鍚姩');
  }
}

export async function setApiKey(provider: string, apiKey: string): Promise<void> {
  return await setProviderConfig({
    provider,
    api_key: apiKey,
  });
}

export async function setProviderConfig(request: ProviderConfigRequest): Promise<void> {
  console.info('[AI] set_provider_config', {
    provider: request.provider,
    apiKeyMasked: request.api_key ? `${request.api_key.slice(0, 4)}***${request.api_key.slice(-2)}` : '',
  ...
  assertTauriReady();
  try {
    return await invoke('set_provider_config', { provider: request.provider, config: request });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const missingCommand = message.includes('set_provider_config') && message.includes('not found');
    if (!missingCommand) {
      throw error;
    }

    // Backward compatibility for an older Tauri binary that only exposes `set_api_key`.
    if (request.api_key !== undefined) {
      await invoke('set_api_key', { provider: request.provider, apiKey: request.api_key });
    }
  }
}
...
export async function chatCompletion(request: ChatCompletionRequestDto): Promise<string> {
  assertTauriReady();
  try {
    return await invoke<string>('chat_completion', { request });
  } catch (error) {
