...
import type { ProviderRuntimeConfigMap } from '@/stores/settingsStore';
import { UiCheckbox, UiInput, UiSelect } from '@/components/ui';
import { UI_CONTENT_OVERLAY_INSET_CLASS, UI_DIALOG_TRANSITION_MS } from '@/components/ui/motion';
import { useDialogTransition } from '@/components/ui/useDialogTransition';
import type { SettingsCategory } from '@/features/settings/settingsEvents';
import { provider as twelveAiTextProvider } from '@/features/canvas/models/providers/12aiText';
import { provider as twelveAiGpt55Provider } from '@/features/canvas/models/providers/12aiGpt55';
import { provider as twelveAiClaudeProvider } from '@/features/canvas/models/providers/12aiClaude';
import { provider as twelveAiGeminiImageProvider } from '@/features/canvas/models/providers/12aiGeminiImage';
import { provider as twelveAiGptImageProvider } from '@/features/canvas/models/providers/12aiGptImage';
import { provider as twelveAiVeoProvider } from '@/features/canvas/models/providers/12aiVeo';
...
export function SettingsDialog({
  ...
  const providers = useMemo(() => SETTINGS_PROVIDERS, []);
  const [activeCategory, setActiveCategory] = useState<SettingsCategory>(initialCategory);
  const [appVersion, setAppVersion] = useState<string>('');
  const [localProviderConfigs, setLocalProviderConfigs] = useState<ProviderRuntimeConfigMap>(
    providerConfigs
  );
  ...
  const handleSave = useCallback(() => {
    providers.forEach((provider) => {
      setProviderConfig(provider.id, localProviderConfigs[provider.id] ?? {
        apiKey: '',
        baseUrl: '',
        model: '',
      });
              ...
              <>
                ...
                <div className="ui-scrollbar flex-1 space-y-4 overflow-y-auto p-6">
                  {providers.map((provider) => {
                    ...
                    const localProviderConfig = localProviderConfigs[provider.id] ?? {
                      apiKey: '',
                      baseUrl: '',
                      model: '',
                    };
                    ...
                    return (
                      <div key={`${provider.id}-provider-config`} className="rounded-lg border border-border-dark bg-bg-dark p-4">
                        <div className="mb-3">
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <UiInput
                              value={localProviderConfig.model}
                              onChange={(event) => {
                                const nextValue = event.target.value;
                                setLocalProviderConfigs((previous) => ({
                                  ...previous,
                                  [provider.id]: {
                                    ...(previous[provider.id] ?? localProviderConfig),
                                    model: nextValue,
                                  },
                                }));
                              }}
                          ...
                          <div>
                            ...
                            <UiInput
                              value={localProviderConfig.baseUrl}
                              onChange={(event) => {
                                const nextValue = event.target.value;
                                setLocalProviderConfigs((previous) => ({
                                  ...previous,
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <UiInput
                                ...
                                value={localProviderConfig.apiKey}
                                onChange={(event) => {
                                  const nextValue = event.target.value;
                                  setLocalProviderConfigs((previous) => ({
                                    ...previous,
                          ...
                          {(provider.id === '12ai-gpt-image' || provider.id === '12ai-gemini-image') && (
                            <div className="pt-2">
                              <SettingsCheckboxCard
                                title={t('settings.preferAsync')}
                                description={t('settings.preferAsyncDesc')}
                                checked={localProviderConfig.preferAsync ?? true}
                                onCheckedChange={(checked) => {
                                  setLocalProviderConfigs((previous) => ({
                                    ...previous,
                                    [provider.id]: {
