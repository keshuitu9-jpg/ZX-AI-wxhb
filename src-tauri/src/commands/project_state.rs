...
fn prune_unreferenced_images(app: &AppHandle) -> Result<(), String> {
    ...
    for entry_result in entries {
        ...
        if !referenced.contains(&path_string) {
            // Skip deletion if the file was modified within the last hour.
            // This prevents race conditions where temporary images (like compressed AI reference images)
            // are deleted by auto-save before the AI background job can read them.
